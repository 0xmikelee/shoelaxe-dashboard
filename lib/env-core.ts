 ...
 * into an image layer.
 *
 * This module has no `server-only` import so the worker (`tsx worker/index.ts`) can parse env
 * without the Next client-guard throw. Next.js routes still import `@/lib/env`, which re-exports
 * these and adds the guard.
 */
const Env = z.object({
